...
/** CI-IC — Jumlah konferensi internasional (archetype D: registry count). */
export default async function CiIcView() {
  const d = await primaryData("CI-IC");
  const rows = d?.rows ?? [];
  ...
  return (
    <IndicatorShell code="CI-IC" source={d?.title}>
      ...
      <Block title="Per jenis usaha (KBLI)"><BarBreakdown data={perJenis} /></Block>
