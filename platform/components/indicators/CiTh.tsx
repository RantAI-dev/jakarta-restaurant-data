...
/** CI-TH — Theaters (archetype E: event list, proksi via venue pertunjukan). */
export default async function CiThView() {
  const d = await primaryData("CI-TH");
  const rows = d?.rows ?? [];
  ...
  return (
    <IndicatorShell code="CI-TH" source={d?.title}>
      ...
      {trend.length >= 2 && (
        <Block title="Tren jumlah pertunjukan per periode"><LineTrend data={trend} /></Block>
      )}
      <Block title="Top venue (jumlah pertunjukan)"><RankedList data={topVenue} /></Block>
      <Block title="Pertunjukan terbaru">
